...
public class ClientCnxn {
    ...
    class SendThread extends Thread {
        ...
        private void startConnect() throws IOException {
            ...
            // Report Healthy on connect
            /*** Send DH report to Panorama Server ***/
            if (client == null){
                client = DHClient.getInstance();
                int client_id = ThreadLocalRandom.current().nextInt(10, 5000 + 1);
                client.init("localhost", 6688, "zookeeper client", Integer.toString(client_id));
            }
            // send the report
            long time = System.currentTimeMillis();
            client.report(time, "2", DHBuilder.NewMetric("network", Health.Status.HEALTHY, 10.0f));
            LOG.info("DHC Report Sent");
        }
                ...
                try {
                    ...
                    if (closing) {
                        ...
                        if (e instanceof SessionExpiredException) {
                            ...
                            // send the report
                            long time = System.currentTimeMillis();
                            client.report(time, "2", DHBuilder.NewMetric("network", Health.Status.UNHEALTHY, -100.0f));
                            LOG.info("DHC Report Sent on SessionTimeoutException");
                            ...
                            long time = System.currentTimeMillis();
                            client.report(time, "2", DHBuilder.NewMetric("network", Health.Status.UNHEALTHY, -100.0f));
                            LOG.info("DHC Report Sent on EndOfStreamException Error");
                            cleanup();
                            // Terminate JVM 
                            System.exit(0); 
                            //LOG.info(e.getMessage() + RETRY_CONN_MSG);
