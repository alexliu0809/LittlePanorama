...
import org.apache.zookeeper.server.quorum.MyControl;
...
import edu.jhu.order.deephealth.DHBuilder;
import edu.jhu.order.deephealth.DHClient;
...
public class LearnerHandler extends Thread {
    ...
    private static DHClient client = null;
    ...
    public void run() {
        try {
            ...
            if (sock != null && !sock.isClosed()) {
                ...
                // Send DH report
                if (client == null){
                    client = DHClient.getInstance();
                    client.init("localhost", 6688, "zookeeper peer", leader.self.getId()+"");
                    LOG.info("DHC Client Initiated");
                }
                /* Send Report to Panorama Server*/
                long time = System.currentTimeMillis();
                // Assuming only 1 is broken here.
                LOG.info("Reporting Follower 1 Failed");
                client.report(time, this.sid+"", DHBuilder.NewMetric("network", Health.Status.UNHEALTHY, -100.0f));
     ...
     */
    public void ping() {
        //LOG.info("Send Heartbeat To Followers");
        try{
            setSpecialNode2();
        } catch (Exception e){
            // do nothing
            LOG.info("setSpecialNode2 Failed");
            LOG.info("setSpecialNode2 Failed");
            LOG.info("setSpecialNode2 Failed");
            LOG.info("setSpecialNode2 Failed");
            LOG.info("setSpecialNode2 Failed");
        ...
        /* Send Report to Panorama Server*/
        long time = System.currentTimeMillis();
        client.report(time, this.sid+"", DHBuilder.NewMetric("network", Health.Status.HEALTHY, 10.0f));
    ...
    /**** New Funcs */
    public void setSpecialNode2() throws Exception{
        if (MyControl.trigger == true && MyControl.triggered == false){
            /*********** Set current leader to faulty state ***********/
            // inject problem in leader
            FileTxnSnapLog snapLog = leader.self.getActiveServer().getTxnLogFactory();
            ...
            ZKDatabase originalZKDatabase = leader.self.getActiveServer()
                    .getZKDatabase();
            long leaderCurrentEpoch = leader.self.getCurrentEpoch();

            ZKDatabase newDB = new ZKDatabase(fileTxnSnapLogWithError);
            leader.self.getActiveServer().setZKDatabase(newDB);
            MyControl.triggered = true;
            LOG.info("Gray2 Injected");
            LOG.info("Gray2 Injected");
            LOG.info("Gray2 Injected");
            LOG.info("Gray2 Injected");
            LOG.info("Gray2 Injected");
